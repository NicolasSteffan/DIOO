...
function executerFiltreSimple(lignes, headers, query) {
    ...
    // Gestion des requêtes SQL simples
    if (query.toLowerCase().includes('select count(*)')) {
        console.log('📊 DataBase - Requête COUNT(*) détectée');
        console.log(`🔍 REQUÊTE SQL DÉTECTÉE: SELECT COUNT(*)`);
        console.log(`📤 ÉQUIVALENT SQL.JS: "SELECT COUNT(*) FROM dioo_donnees"`);
        
        // Extraire les conditions WHERE si présentes
        const whereMatch = query.match(/where\s+(.+?)(?:;|$)/i);
        let lignesFiltrees = lignes;
        
        if (whereMatch) {
            const whereClause = whereMatch[1].trim();
            console.log(`🔍 DataBase - Clause WHERE: "${whereClause}"`);
            console.log(`📤 ÉQUIVALENT SQL.JS COMPLET: "SELECT COUNT(*) FROM dioo_donnees WHERE ${whereClause}"`);
            
            // Parser simple pour les conditions
            lignesFiltrees = filtrerLignesAvecWhere(lignes, headers, whereClause);
        } else {
            console.log(`📤 ÉQUIVALENT SQL.JS FINAL: "SELECT COUNT(*) FROM dioo_donnees" (sans WHERE)`);
        }
        
        const count = lignesFiltrees.length;
        console.log(`✅ DataBase - COUNT(*) résultat: ${count}`);
        console.log(`📊 RÉSULTAT FINAL: COUNT(*) = ${count}`);
        
        // Retourner le résultat avec un nom de colonne plus clair
        return [{ 'Nombre de lignes': count }];
    }
    
    // Gestion des requêtes avec "filtrer par"
    if (query.toLowerCase().includes('filtrer par')) {
        console.log('🔍 DataBase - Requête "filtrer par" détectée');
        
        const match = query.match(/\[([^\]]+)\]\s*=\s*['"']([^'"']+)['"']/i);
        if (match) {
            const [, colonne, valeur] = match;
            const colonneIndex = trouverIndexColonne(headers, colonne);
            
            console.log(`🔍 DataBase - Filtre: colonne="${colonne}", valeur="${valeur}", index=${colonneIndex}`);
            
            if (colonneIndex !== -1) {
                const lignesFiltrees = lignes.filter(ligne => {
                    const valeurLigne = ligne[headers[colonneIndex]] || '';
                    return valeurLigne.toString().toUpperCase() === valeur.toUpperCase();
                });
                
                console.log(`✅ DataBase - ${lignesFiltrees.length} lignes filtrées`);
                
                // Retourner toutes les lignes filtrées avec toutes les colonnes
                return lignesFiltrees.map(ligne => {
                    const resultat = {};
                    headers.forEach(header => {
                        // Les données sont stockées comme des objets, pas des tableaux
                        resultat[header] = ligne[header] || '';
                    });
                    return resultat;
                });
            }
    ...
    // Gestion des requêtes SELECT sans COUNT
    if (query.toLowerCase().includes('select') && !query.toLowerCase().includes('count')) {
        console.log('📋 DataBase - Requête SELECT détectée (sans COUNT)');
        
        // Extraire les colonnes spécifiées dans SELECT
        const selectMatch = query.match(/select\s+(.+?)\s+from/i);
        let colonnesSelectionnees = [];
        let toutesLesColonnes = false;
        
        if (selectMatch) {
            const colonnesStr = selectMatch[1].trim();
            console.log(`🔍 DataBase - Colonnes SELECT brutes: "${colonnesStr}"`);
            
            if (colonnesStr === '*') {
                toutesLesColonnes = true;
                colonnesSelectionnees = headers;
                console.log('📋 DataBase - SELECT * détecté - toutes les colonnes');
            } else {
                // Parser les colonnes spécifiées
                const colonnesBrutes = colonnesStr.split(',').map(col => col.trim());
                console.log('🔍 DataBase - Colonnes brutes:', colonnesBrutes);
                
                colonnesSelectionnees = colonnesBrutes.map(colonneBrute => {
                    // Supprimer les crochets si présents
                    let nomColonne = colonneBrute.replace(/^\[|\]$/g, '');
                    
                    // Chercher la colonne correspondante dans les headers
                    const colonneCorrespondante = headers.find(header => 
                        header.toLowerCase() === nomColonne.toLowerCase() ||
                        header.toLowerCase().includes(nomColonne.toLowerCase())
                    );
                    
                    if (colonneCorrespondante) {
                        console.log(`✅ DataBase - Colonne "${nomColonne}" -> "${colonneCorrespondante}"`);
                        return colonneCorrespondante;
                    } else {
                        console.log(`⚠️ DataBase - Colonne "${nomColonne}" non trouvée`);
                        return nomColonne; // Garder le nom original même si non trouvé
                    }
                }).filter(Boolean);
                
                console.log('📋 DataBase - Colonnes sélectionnées:', colonnesSelectionnees);
            }
        } else {
            // Si on ne peut pas parser, utiliser toutes les colonnes
            toutesLesColonnes = true;
            colonnesSelectionnees = headers;
            console.log('⚠️ DataBase - Impossible de parser SELECT, utilisation de toutes les colonnes');
        }
        ...
        if (whereMatch) {
            const whereClause = whereMatch[1].trim();
            console.log(`🔍 DataBase - Clause WHERE pour SELECT: "${whereClause}"`);
            lignesFiltrees = filtrerLignesAvecWhere(lignes, headers, whereClause);
        }
        
        console.log(`📊 DataBase - ${lignesFiltrees.length} lignes trouvées pour SELECT`);
        
        if (lignesFiltrees.length === 0) {
            console.log('⚠️ DataBase - Aucune ligne trouvée pour SELECT');
            return [];
        }
        
        // Retourner les lignes avec seulement les colonnes sélectionnées
        console.log(`🚨 EXECUTER_FILTRE - Début du mapping SELECT`);
        console.log(`🚨 EXECUTER_FILTRE - Colonnes à retourner:`, colonnesSelectionnees);
        console.log(`🚨 EXECUTER_FILTRE - Première ligne brute:`, lignesFiltrees[0]);
        
        return lignesFiltrees.map((ligne, index) => {
            console.log(`🚨 EXECUTER_FILTRE - Ligne ${index}:`, ligne);
            const resultat = {};
            colonnesSelectionnees.forEach(colonne => {
                const valeur = ligne[colonne] || '';
                resultat[colonne] = valeur;
                console.log(`🚨 EXECUTER_FILTRE - ${colonne}: "${valeur}"`);
            });
            console.log(`🚨 EXECUTER_FILTRE - Résultat ligne ${index}:`, resultat);
            return resultat;
        });
    }
    
    // Par défaut, retourner les premières lignes
    console.log('📋 DataBase - Retour des premières lignes par défaut');
    return lignes.slice(0, 10).map(ligne => {
        const resultat = {};
...
function filtrerLignesAvecWhere(lignes, headers, whereClause) {
    ...
    // Parser simple pour des conditions comme:
                    // [Dx] = 'DP' AND [Business criticality] = 'Critical'
                    // [Functional monitoring (BSM)] = 'YES'
    
    const conditions = whereClause.split(/\s+and\s+/i);
    console.log(`🔍 DataBase - ${conditions.length} condition(s) trouvée(s):`, conditions);
    
    return lignes.filter(ligne => {
        return conditions.every(condition => {
            const match = condition.match(/\[([^\]]+)\]\s*=\s*['"']([^'"']+)['"']/i);
            if (match) {
                const [, colonne, valeur] = match;
                const colonneIndex = trouverIndexColonne(headers, colonne);
                
                if (colonneIndex !== -1) {
                    const valeurLigne = ligne[colonneIndex] || '';
                    const resultat = valeurLigne.toString().toUpperCase() === valeur.toUpperCase();
                    return resultat;
                }
            }
            return true; // Si on ne peut pas parser la condition, on l'ignore
        });
    });
}
...
function creerRequeteSQLJS(query, headers) {
    ...
    if (queryLower.includes('select count(*)')) {
        ...
        if (whereMatch) {
            ...
            // Convertir les noms de colonnes avec crochets en noms SQL valides
            let sqlWhereClause = whereClause;
            
            // Remplacer [nom_colonne] par "nom_colonne_exact"
            sqlWhereClause = sqlWhereClause.replace(/\[([^\]]+)\]/g, (match, columnName) => {
                ...
                if (columnIndex !== -1) {
                    // Utiliser le nom exact de la colonne avec des guillemets
                    return `"${headers[columnIndex]}"`;
                } else {
                    // Fallback: utiliser le nom tel quel avec des guillemets
                    return `"${columnName}"`;
                }
            });
            
            sqlQuery += ` WHERE ${sqlWhereClause}`;
        }
        ...
        if (whereMatch) {
            ...
            // Convertir les noms de colonnes avec crochets
            let sqlWhereClause = whereClause;
            sqlWhereClause = sqlWhereClause.replace(/\[([^\]]+)\]/g, (match, columnName) => {
                const columnIndex = headers.findIndex(h => 
                    h.toLowerCase().includes(columnName.toLowerCase())
        ...
        // Pas de limite - afficher toutes les lignes
    } else if (queryLower.includes('select')) {
        // Autres requêtes SELECT complexes
        sqlQuery = query.replace(/dioo_donnees/gi, 'dioo_donnees');
        
        // Convertir les crochets en guillemets
        sqlQuery = sqlQuery.replace(/\[([^\]]+)\]/g, (match, columnName) => {
            const columnIndex = headers.findIndex(h => 
                h.toLowerCase().includes(columnName.toLowerCase())
            );
        ...
        // Requête non-SQL, créer un équivalent SELECT
        sqlQuery = 'SELECT * FROM dioo_donnees LIMIT 10';
    }
    
    // Nettoyer la requête
    sqlQuery = sqlQuery.replace(/;+$/, ''); // Supprimer les points-virgules en fin
    sqlQuery += ';'; // Ajouter un seul point-virgule
    
    console.log(`📤 REQUÊTE SQL.JS GÉNÉRÉE: "${sqlQuery}"`);
    return sqlQuery;
}
